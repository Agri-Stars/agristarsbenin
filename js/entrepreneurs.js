  ...
  liste.forEach(e => {
    ...
    card.innerHTML = `
      ...
      <p class="bio line-clamp-2">${e.bio || 'Entrepreneur agricole béninois.'}</p>
            <button type="button"
              class="wa btn-contact-entrepreneur"
              data-id="${e.id || ''}"
              data-nom="${e.nom}"
              data-whatsapp="${e.whatsapp}"
              data-commune="${e.commune || ''}"
              data-activite="${e.activite || ''}"
...
load();
// ===== GESTION DU MODAL DE CAPTURE LEADS =====

// Ouvrir le modal au clic sur "Contacter"
document.addEventListener('click', function(e) {
  const btn = e.target.closest('.btn-contact-entrepreneur');
  if (!btn) return;
  
  e.preventDefault();
  
  // Récupérer les données de l'entrepreneur depuis les data-*
  const entrepreneur = {
    id: btn.dataset.id,
    nom: btn.dataset.nom,
    whatsapp: btn.dataset.whatsapp,
    commune: btn.dataset.commune,
    activite: btn.dataset.activite
  };
  
  // Pré-remplir les champs cachés du modal
...
// Ouvrir le modal
function openLeadModal(entrepreneur) {
  ...
  // Focus sur le premier champ pour accessibilité
  setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('modal-overlay')?.addEventListener('click', closeLeadModal);
  ...
  document.getElementById('lead-form')?.addEventListener('submit', function(e) {
    ...
    if (typeof gtag === 'function') {
      gtag('event', 'lead_submit', {
        ...
        'event_label': entrepreneurNom || 'entrepreneur',
        'visitor_name': nom,
        'visitor_type': type,
        'value': 1
      });
...
const leadData = {
  ...
  type_lead: 'Entrepreneur',
  nom_visiteur: nom,
  contact_visiteur: contact,
  type_demande: type,
  cible_nom: entrepreneurNom,
  cible_whatsapp: entrepreneurWhatsapp,
  quantite: '',
  message: message,
...
const whatsappMessage = 
  `Bonjour ${entrepreneurNom},%0A%0A` +
  `Je vous contacte depuis AgriStars Bénin.%0A%0A` +
  `👤 Nom : ${nom}%0A` +
  `📞 Contact : ${contact}%0A` +
  `🎯 Type : ${type}%0A` +
  ...
  `https://agri-stars.github.io/agristarsbenin`;
    ...
    // 4. Ouvrir WhatsApp avec le message pré-rempli
    const whatsappUrl = `https://wa.me/${entrepreneurWhatsapp}?text=${whatsappMessage}`;
    window.open(whatsappUrl, '_blank', 'noopener');
    
    // 5. Fermer le modal
    closeLeadModal();
    
    // 6. Afficher un message de confirmation (optionnel)
    alert('✅ Redirection vers WhatsApp...\n\nLe message contient vos informations et la mention "Depuis AgriStars Bénin".');
  });
});
